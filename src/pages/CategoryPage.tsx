...
export default function CategoryPage() {
  ...
  if (!catInfo) {
    return <div className="min-h-screen bg-[#F9F4EE] pt-24 pb-16"><div className="mx-auto max-w-[84rem] px-10 py-20"><p className="text-stone-600">Category not found.</p></div></div>
  }

  return (
    <div className="min-h-screen bg-[#F9F4EE] pt-24 pb-16">
      <div className="mx-auto max-w-[84rem] px-6 sm:px-10 lg:px-12">
        <Link to="/services" className="font-sans text-[10px] font-medium uppercase tracking-[0.18em] text-stone-600 hover:text-stone-600">&larr; All Categories</Link>
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((p) => (
            <Link key={p.id} to={`/services/${p.categorySlug}/${p.slug}`} className="group block border border-stone-300 bg-white/60 overflow-hidden transition hover:border-stone-400">
              ...
              <div className="p-5">
                ...
                <p className="mt-2 text-[12px] text-stone-600">{p.colorType} &middot; {p.suitablePlacements}</p>
